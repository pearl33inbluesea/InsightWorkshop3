...
function showNextSlide() {
    slides[currentSlide].style.transform = 'translateX(-100%)'; // 현재 슬라이드를 왼쪽으로 밀어냄
    ...
    slides[currentSlide].style.transform = 'translateX(0)'; // 다음 슬라이드를 제자리로 옮김
...
slides.forEach((slide, index) => {
    if (index !== 0) {
        slide.style.transform = 'translateX(100%)'; // 초기 상태에서 화면 오른쪽으로 배치
    }
});
